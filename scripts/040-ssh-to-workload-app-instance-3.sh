...
WORKLOAD_APP_INSTANCE_3_PRIVATE_IP=$(aws cloudformation describe-stacks \
  --stack-name inf3701-extra-points \
  --query 'Stacks[0].Outputs[?OutputKey==`WorkloadAppInstance3PrivateIp`].OutputValue' \
  --output text)
...
ssh -A -i ssh-private_key.pem -J ec2-user@$BASTION_HOST_PUBLIC_IP ec2-user@$WORKLOAD_APP_INSTANCE_3_PRIVATE_IP
